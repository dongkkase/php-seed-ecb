...
class Seed
{
    ...
    /***************************** SEED round function ****************************/
    /**
    * @param L0,L1 [in, out] left-side variable at each round
    * @param R0,R1 [in] right-side variable at each round
    * @param k [in] round keys at each round
    */
    public function SeedRound(&$L0, &$L1, &$R0, &$R1, $K = array())
    {
        $T0 = $R0 ^ $K[0];
        $T1 = $R1 ^ $K[1];
        $T1 ^= $T0;
    ...
    /**
    * @param pbData [in] data to be encrypted
    * @param pdwRoundKey [in] round keys for encryption
    * @param outData [out] encrypted data
    */
    public function SeedEncrypt($pbData = array(), $pdwRoundKey = array(), &$outData = array())
    {
    ...
    /**
    * @param pbData [in] encrypted data
    * @param pdwRoundKey [in] round keys for decryption
    * @param outData [out] data to be encrypted
    */
    public function SeedDecrypt($pbData = array(), $pdwRoundKey = array(), &$outData = array())
    {
        $L0 = 0x0;
        $L1 = 0x0;
        $R0 = 0x0;
    ...
    private function EncRoundKeyUpdate0(&$K = array(), &$A, &$B, &$C, &$D, $Z)
    {
        ...
        $A = ( $A >> 8 & 0xffffffff ) ^ ( $B << 24 & 0xffffffff);
        $B = ( $B >> 8 & 0xffffffff ) ^ ( $T0 << 24 & 0xffffffff);
    ...
    private function EncRoundKeyUpdate1(&$K = array(), &$A, &$B, &$C, &$D, $Z)
    {
        ...
        $T0 = $C;
        $C = ( $C << 8 & 0xffffffff) ^ ( $D >> 24  & 0xffffffff);
        $D = ( $D << 8 & 0xffffffff) ^ ( $T0 >> 24  & 0xffffffff);
    ...
    /******************************** Key Schedule ********************************/
    /**
    * @param pdwRoundKey [out] round keys for encryption or decryption
    * @param pbUserKey [in]    secret user key
    */
    public function SeedRoundKey(&$pdwRoundKey = array(), $pbUserKey = array())
    {
        $K = array();
        $nCount = 2;
